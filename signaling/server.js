...
const STR_LOBBY_DOES_NOT_EXISTS = 'Lobby does not exist';
...
class Lobby {
	...
	leave(peer) {
		...
		this.peers.forEach((p) => {
			...
			else p.ws.send(ProtoMessage(CMD.PEER_DISCONNECT, assigned));
		});
	...
	seal(peer) {
		if (peer.id !== this.host) throw new ProtoError(4000, STR_ONLY_HOST_CAN_SEAL);
		this.sealed = true;
		this.peers.forEach((p) => p.ws.send(ProtoMessage(CMD.SEAL, 0)));
		console.log(`Peer ${peer.id} sealed lobby ${this.name} with ${this.peers.length} peers`);
		this.closeTimer = setTimeout(() => {
			this.peers.forEach((p) => p.ws.close(1000, STR_SEAL_COMPLETE));
		}, SEAL_CLOSE_TIMEOUT);
	}
}
...
function joinLobby(peer, pLobby, mesh) {
	...
	if (lobbyName === '') {
		if (lobbies.size >= MAX_LOBBIES) throw new ProtoError(4000, STR_TOO_MANY_LOBBIES);
		if (peer.lobby !== '') throw new ProtoError(4000, STR_ALREADY_IN_LOBBY);
		lobbyName = randomSecret();
		lobbies.set(lobbyName, new Lobby(lobbyName, peer.id, mesh));
		console.log(`Peer ${peer.id} created lobby ${lobbyName} | open: ${lobbies.size}`);
	}
	...
	console.log(`Peer ${peer.id} joining lobby ${lobbyName} with ${lobby.peers.length} existing peers`);
	lobby.join(peer);
...
function parseMsg(peer, msg) {
	let json = null;
	try { json = JSON.parse(msg); } catch (e) { throw new ProtoError(4000, STR_INVALID_FORMAT); }
	...
	if (type < 0 || id < 0) throw new ProtoError(4000, STR_INVALID_FORMAT);

	if (type === CMD.JOIN) { joinLobby(peer, data, id === 0); return; }
	...
	if (type === CMD.SEAL) { lobby.seal(peer); return; }

	if (type === CMD.OFFER || type === CMD.ANSWER || type === CMD.CANDIDATE) {
		let destId = id === 1 ? lobby.host : id;
...
const server = http.createServer((_req, res) => {
	res.writeHead(200);
	res.end('Mobber Signaling Server\n');
});

const wss = new WebSocket.Server({ server });

wss.on('connection', (ws) => {
	if (peersCount >= MAX_PEERS) { ws.close(4000, STR_TOO_MANY_PEERS); return; }
	peersCount++;
	const id = randomId();
	const peer = new Peer(id, ws);

	ws.on('message', (message, isBinary) => {
		if (isBinary) { ws.close(4000, STR_INVALID_TRANSFER_MODE); return; }
		try {
			parseMsg(peer, message.toString());
		} catch (e) {
			console.log(`Error from peer ${id}: ${e.message}`);
			ws.close(e.code || 4000, e.message);
		}
	});

	ws.on('close', (code, reason) => {
		peersCount--;
		console.log(`Peer ${peer.id} disconnected: ${code} ${reason}`);
		if (peer.lobby && lobbies.has(peer.lobby) && lobbies.get(peer.lobby).leave(peer)) {
			lobbies.delete(peer.lobby);
			console.log(`Deleted lobby ${peer.lobby} | open: ${lobbies.size}`);
			peer.lobby = '';
		}
		if (peer.timeout >= 0) { clearTimeout(peer.timeout); peer.timeout = -1; }
	});
